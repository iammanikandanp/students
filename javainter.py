...
u = """select java from intercal"""
cur.execute(u)
now = cur.fetchall()

o = """select javai from model"""
cur.execute(o)
wow = cur.fetchall()
for x in hot:
    print("""
...
if sumbit != None:
    for (i, x, y, z, k) in itertools.zip_longest(hot, now, wow, sem, att):
        ...
        t = iny + moe + sem + att
        n = round(t)
        tr = """INSERT INTO javainter(name,regno,internal,model,sem,attendance, total) values ('%s','%s','%s','%s','%s','%s','%s')""" % (a[0], a[1], d, e, z, k, n)
        cur.execute(tr)
        con.commit()
        print("""
        <script>
