...
o="""select regno,name from internal2 """
cur.execute(o)
cow=cur.fetchall()
    ...
    for (i, y, z, k,v) in itertools.zip_longest(hot, ml, wb, java,cow):
        ...
        if v[0] != i[0]:
            if (mli and wbi and javai) <= 2.5:
                jl = """insert into internal2 (regno,name,ml,wd,java,mli,wdi,javai) values ('%s','%s','%s','%s','%s','%s','%s','%s') """ % (i[0], i[1], y, z, k, mli, wbi, javai)
                cur.execute(jl)
                con.commit()
                print("""<script>
        ...
        if v[0] == i[0]:
            ui="""update internal2 set ml='%s',wd='%s',java='%s',mli='%s',wdi='%s',javai='%s' where regno='%s'"""%(y, z, k, mli, wbi, javai,i[0])
            cur.execute(ui)
            con.commit()
            print("""<script>
