...
<body>
    ...
    <div class="hg">
    <h1 class="ty">1 INTERNAL MARKSHEET</h1>
    </div>
    <div class="container">
      <form enctype = "multipart/form-data" method = "post">   
    <div class="mm">
    <table class="table table-hover table-striped table-bordered kl">
        <tr>
            <th>ROLL NO</th>
            <th>NAME</th>
...
o="""select rego,name from internal """
cur.execute(o)
cow=cur.fetchall()
if submit != None:
    for (i,y,z,k ,v) in itertools.zip_longest(hot,ml,wb,java,cow):
        ...
        javai=int(k)/10
        if v[0]!=i[0]:
            if (mli and wbi and javai) <= 2.5:
                jl = """insert into internal (rego,name,ml,wd,java,mli,wdi,javai) values ('%s','%s','%s','%s','%s','%s','%s','%s') """ % ( i[0], i[1], y, z, k, mli, wbi, javai)
                cur.execute(jl)
                con.commit()
                print("""<script>
        ...
        if v[0] == i[0]:
            ui="""update internal set ml='%s',wd='%s',java='%s',mli='%s',wdi='%s',javai='%s' where rego='%s'"""%(y, z, k, mli, wbi, javai,i[0])
            cur.execute(ui)
            con.commit()
            print("""<script>
    ...
    document.addEventListener("DOMContentLoaded",function(){
        ...
        inputs.forEach(input =>{
            input.addEventListener("input",function(){
                ...
                if (/[^0-9]/.test(values)||(values && (Number(values) < 0 || Number(values) > 25))){
                    alert("Marks upto 25. Invailed characters are not allowed ")
                    input.value=""
                }else{
