...
<body>
   <nav>
    ...
    <div class="hg">
    <h1 class="ty">3 INTERNAL MARKSHEET</h1>
    </div>
...
o="""select regno,name from internal3 """
cur.execute(o)
cow=cur.fetchall()
if submit != None:
    for (i, y, z, k,v) in itertools.zip_longest(hot, ml, wb, java,cow):
        ...
        if v[0] != i[0]:
            if (mli and wbi and javai) <= 2.5:
                jl = """insert into internal3 (regno,name,ml,wd,java,mli,wdi,javai) values ('%s','%s','%s','%s','%s','%s','%s','%s') """ % (i[0],i[1], y, z, k, mli, wbi, javai)
                cur.execute(jl)
                con.commit()
                print(jl)
                print("""<script>
        ...
        if v[0] == i[0]:
            ui="""update internal3 set ml='%s',wd='%s',java='%s',mli='%s',wdi='%s',javai='%s' where regno='%s'"""%(y, z, k, mli, wbi, javai,i[0])
            cur.execute(ui)
            con.commit()
            print("""<script>
