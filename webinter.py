...
<head>
    ...
    <title>WEB TECH SEM INTERNAL</title>
...
u = """select wb from intercal"""
cur.execute(u)
now = cur.fetchall()

o = """select wdi from model"""
cur.execute(o)
wow = cur.fetchall()
for x in hot:
    print("""
...
if sumbit != None:
    for (i, x, y, z, k) in itertools.zip_longest(hot, now, wow, sem, att):
        a = list(i)
        b = list(x)
        for d in b:
            pass
        ...
        t = iny + moe + sem + att
        n = round(t)
        tr = """INSERT INTO webinter(name,regno,internal,model,sem,attendance, total) values ('%s','%s','%s','%s','%s','%s','%s')""" % (a[0], a[1], d, e, z, k, n)
        cur.execute(tr)
        con.commit()
        print("""
        <script>
